...
func (lh *loanHandler) UpdateLoanStatusByID(c echo.Context) error {
	loanID := c.Param("id")
	if loanID == "" {
	...
	existingLoan, errGetLoan := lh.loanQueryUsecase.GetLoanByID(loanID)
	if errGetLoan != nil {
		return c.JSON(http.StatusNotFound, response.ErrorResponse(constant.ERROR_ID_NOTFOUND))
	}

	if existingLoan.UserID != tokenUserID {
		return c.JSON(http.StatusUnauthorized, response.ErrorResponse(constant.ERROR_ROLE_ACCESS))
	}

	loanUpdateRequest := dto.LoanUpdateRequest{}
	errBind := c.Bind(&loanUpdateRequest)
	if errBind != nil {
		return c.JSON(http.StatusBadRequest, response.ErrorResponse(errBind.Error()))
	}

	updatedLoanDomain := dto.LoanUpdateRequestToLoanDomain(loanUpdateRequest)

	_, errUpdate := lh.loanCommandUsecase.UpdateLoanStatusByID(loanID, updatedLoanDomain)
	if errUpdate != nil {
